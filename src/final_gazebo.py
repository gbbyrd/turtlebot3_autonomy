...
class Final(object):
    ...
    def camera_callback(self, msg):
        try:
            self.cv2_image = cv2.resize(self.bridge_object.compressed_imgmsg_to_cv2(msg,desired_encoding="rgb8"), (400, 200), interpolation=cv2.INTER_AREA)
    ...
    def convert_binary(self):
        ...
        # convert to hsv
        hsv_image = cv2.cvtColor(self.cv2_image, cv2.COLOR_RGB2HSV)
    ...
    def line_pid_control(self, error):
        ...
        # when tuning, it was found that the integral and derivate were not necessary
        total = P*self.line_Kp # + self.I*self.Ki + D*self.Kd
        return total
